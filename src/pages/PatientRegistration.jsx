...
import {
  getAllTodayVisits,
  registerPatientAndCreateVisit,
  updatePatientAndCreateVisit,
  createVisit,
} from "../api/services/visitService.js";
import {
  getPatientByPhone,
  updatePatientDetails,
} from "../api/services/patientService.js";
...
const PatientRegistration = () => {
  ...
  const patientsPending = todayVisits?.filter(
    (patient) => patient.status === "Pending"
  );
  const patientsInConsultation = todayVisits?.filter(
    (patient) => patient.status === "In Consultation"
  );
  const patientsOnHold = todayVisits?.filter(
    (patient) => patient.status === "Hold"
  );
  const patientsCompleted = todayVisits?.filter(
  ...
  const handleSubmit = async () => {
    ...
    if (editPatientId) {
      ...
      try {
        ...
        setShowMessage(
          `Patient updated successfully with token #${updatedPatient.tokenNo}`
        );
      ...
      try {
        ...
        setShowMessage(
          `Patient registered successfully with token #${PatientVisit.tokenNo}`
        );
        setTimeout(() => setShowMessage(null), 3000);
      } catch (error) {
        console.error("❌ Error:", error.response?.data || error.message);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        ...
        <div className="rounded-xl p-4 shadow-sm bg-yellow-50 border border-yellow-100 flex flex-col justify-between">
          <h3 className="text-3xl font-semibold text-yellow-900">
        ...
        <div className="rounded-xl p-4 shadow-sm bg-red-50 border border-red-100 flex flex-col justify-between">
          <h3 className="text-3xl font-semibold text-red-900">
            {patientsOnHold?.length || "0"}
          </h3>
                ...
                >
                  ...
                  <div className="flex justify-between">
                    <div className="text-xs text-gray-500 mt-1">
                      ...
                      {new Date(visit?.registrationDate).toLocaleTimeString(
                        `un-PK`,
                        {
                          timeStyle: "short",
                          hour12: true,
